...
test("根 del 可以删除没有同名账号的 relay 槽位", async () => {
  const homeDir = createIsolatedHome();
  const cslotDir = path.join(homeDir, ".cslot");
  const configPath = path.join(cslotDir, "config.yaml");
  const statePath = path.join(cslotDir, "state.json");
      ...
      accounts: [
        {
          id: "yuan20x",
          name: "yuan20x",
          codex_home: path.join(cslotDir, "homes", "yuan20x"),
          enabled: true
        }
      ],
      relay_slots: [
  ...
  try {
    const result = await runCli(homeDir, ["del", "moyuu"]);
    const config = YAML.parse(fs.readFileSync(configPath, "utf8"));
    const state = JSON.parse(fs.readFileSync(statePath, "utf8"));

    assert.match(result.stdout, /已删除中转槽位: moyuu|Relay slot removed: moyuu/);
    assert.deepEqual(config.accounts.map((account) => account.id), ["yuan20x"]);
    assert.deepEqual(config.relay_slots, []);
    assert.deepEqual(state.selected_model_route, {
      mode: "auth_pool"
    });
...
test("根 rename 可以重命名没有同名账号的 relay 槽位", async () => {
  const homeDir = createIsolatedHome();
  const cslotDir = path.join(homeDir, ".cslot");
  const configPath = path.join(cslotDir, "config.yaml");
  const statePath = path.join(cslotDir, "state.json");
  ...
  try {
    const result = await runCli(homeDir, ["rename", "moyuu", "backup"]);
    const config = YAML.parse(fs.readFileSync(configPath, "utf8"));
    const state = JSON.parse(fs.readFileSync(statePath, "utf8"));

    assert.match(result.stdout, /已重命名中转槽位: moyuu -> backup|Relay slot renamed: moyuu -> backup/);
    assert.deepEqual(config.accounts.map((account) => account.id), ["yuan20x"]);
    assert.equal(config.relay_slots[0].id, "backup");
    assert.equal(config.relay_slots[0].name, "backup");
    assert.equal(config.relay_slots[0].api_key, "relay-secret");
    assert.deepEqual(state.selected_model_route, {
      mode: "relay_slot",
      relay_slot_id: "backup"
    });
  } finally {
    fs.rmSync(homeDir, { recursive: true, force: true });
  }
